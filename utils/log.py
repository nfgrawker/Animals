"""logger class for printing and logging at one time. """
import logging
...
class Logger:
    ...
    def info(self, content: Any) -> None:
        ...
        secho(f"{style('## info', fg='cyan'):{' '}{'<'}{12}} " f" - " f"{content}")
        logging.info(content)

    def error(self, content: Any) -> None:
        """Use this for error messages."""
        secho(f"{style('.: error', fg='red'):{' '}{'<'}{12}}" f" - " f"{content}")
        logging.error(content)

    def warn(self, content: Any) -> None:
        """Use this for warning messages."""
        secho(f"{style('** warn ', fg='yellow'):{' '}{'<'}{12}}" f" - " f"{content}")
        logging.warning(content)

    def debug(self, content: Any) -> None:
        """Use this for debug messages."""
        secho(f"{style('?? debug', fg='green'):{' '}{'<'}{12}}" f" - " f"{content}")
